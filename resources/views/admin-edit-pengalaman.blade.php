            ...
            <div class="layout-page">
                ...
                <div class="content-wrapper">
                    ...
                    <div class="container-xxl flex-grow-1 container-p-y">
                        <h4 class="py-3 mb-4"><span class="text-muted fw-light">Tables /</span> Edit Pengalaman</h4>

                        <!-- Basic Forms -->
                        <div class="card">
                            <h5 class="card-header">Edit Pengalaman</h5>
                            <div class="card-body">
                                <form action="{{ route('admin.pengalaman.update', $pengalaman->id) }}" method="POST">
                                    @csrf
                                    @method('PUT')
                                    <!-- Form fields for editing an experience -->
                                    <div class="mb-3">
                                        <label for="id" class="form-label">Gunung</label>
                                        <select class="form-select" id="id" name="id" required>
                                            <option value="">Pilih Gunung</option>
                                            @foreach ($gunungs as $gunung)
                                            <option value="{{ $gunung->id }}" {{ $gunung->id == $pengalaman->id ? 'selected' : '' }}>
                                                {{ $gunung->nama }}
                                            </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="text" class="form-control" id="nama" name="nama" value="{{ $pengalaman->nama }}" required>
                                    </div>
                                    <div class="mb-3">
                                        <label for="tanggal" class="form-label">Tanggal</label>
                                        <input type="date" class="form-control" id="tanggal" name="tanggal" value="{{ $pengalaman->tanggal }}" required>
                                    </div>
                                    <div class="mb-3">
